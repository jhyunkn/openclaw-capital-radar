...
check(html.includes('id="holdings-section"') && html.includes('id="opportunities-section"'), 'homepage missing canonical sections that absorb gate state');
...
const output = {
  ...
  checks: { tickers:audit.counts?.tickers || 0, pass:audit.counts?.pass || 0, blocked:audit.counts?.blocked || 0, primaryEvidenceBlocked:list(audit.blockedByGate?.primaryEvidence).length, dashboardPanel:hasCanonicalHomepage(html), publicSync:hasCanonicalHomepage(publicHtml) },
  failures
};
