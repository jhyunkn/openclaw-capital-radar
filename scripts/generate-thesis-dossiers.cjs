...
function textOf(x){ return `${x.ticker || ''} ${x.name || ''} ${x.exposureBucket || ''} ${x.role || ''} ${x.thesis || ''} ${x.actionRationale || ''} ${(x.confirmBeforeAdd || []).join(' ')} ${(x.keyRisks || []).join(' ')}`; }
...
function inferMacroLinkage(x, interp){
  ...
  return links.length ? links : ['Macro linkage not sufficiently mapped; requires research.'];
}
...
function risks(x, interp, candidate){
  ...
  if (interp?.portfolioConflict?.tone !== 'positive') items.push(interp.portfolioConflict.reason);
...
function confidence(x, interp, dossier){
  ...
  if (dossier.confirmBeforeAdd.length >= 3) score += 8;
  if (dossier.keyRisks.length >= 3) score += 7;
...
function dossierFor(x, type){
  ...
  const dossier = {
    ...
    trimCondition: list(interp.signalChangeConditions).find(v => /trim|rebalance/i.test(v)) || 'Trim if valuation, technical structure, or risk budget becomes unfavorable.',
    exitCondition: list(interp.signalChangeConditions).find(v => /exit|break|invalidation/i.test(v)) || note?.agentThesis?.invalidation || x.watch || 'Exit condition requires explicit invalidation rule.',
    keyRisks: risks(x, interp, candidate),
    dataGaps: interp?.dataConfidence?.missing || [],
...
const holdingDossiers = holdings.map(h => dossierFor(h, 'holding'));
const candidateDossiers = candidates.filter(c => !holdings.some(h => String(h.ticker || '').toUpperCase() === String(c.ticker || '').toUpperCase())).map(c => dossierFor(c, 'candidate'));
const result = { generatedAt: new Date().toISOString(), purpose: 'Source object for deeper thesis analysis across holdings and research candidates.', priority: 'Analytical depth, strategic usefulness, data integrity.', holdings: holdingDossiers, candidates: candidateDossiers, all: [...holdingDossiers, ...candidateDossiers] };
