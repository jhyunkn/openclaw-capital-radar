...
(function(){
  ...
  function init(data){
    ...
    var z=data.zones;
    // Buy zone: colored lines, no axis labels (numbers live in levels strip below — avoid right-axis crowd)
    ...
    // Stop: solid red on right axis — only axis label kept (most critical threshold)
    if(z.stop!=null){
      cs.createPriceLine({price:z.stop,color:'#A4502F',lineWidth:1.5,lineStyle:0,axisLabelVisible:true,title:'Stop'});
    }
    // Substanzwert floor: dashed indigo line + watermark label inside the canvas
    if(z.floorPrice!=null){
      cs.createPriceLine({price:z.floorPrice,color:'rgba(100,80,180,.6)',lineWidth:2,lineStyle:2,axisLabelVisible:false,title:'Floor'});
      chart.applyOptions({watermark:{visible:true,fontSize:11,horzAlign:'left',vertAlign:'bottom',color:'rgba(100,80,180,.72)',text:'Floor  $'+z.floorPrice.toFixed(0)+'  ·  Substanzwert (15× EPS)'}});
    }
