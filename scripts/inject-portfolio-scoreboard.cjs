...
function status(score) { if (score >= 70) return 'supportive'; if (score >= 45) return 'mixed'; return 'weak'; }
function fmtPct(value) { return Number.isFinite(value) ? `${value >= 0 ? '+' : ''}${value.toFixed(2)}%` : 'missing'; }
function authoritativeSignal(h) {
  const signal = String(h.computedSignal || h.signal || 'Review').toUpperCase();
  if (signal.includes('EXIT')) return { state: 'exit_review', label: 'Exit review', scoreCap: 44, uiClass: 'bad', reason: 'Authoritative signal is EXIT REVIEW; support labels are suppressed.' };
  if (signal.includes('TRIM')) return { state: 'trim_watch', label: 'Trim watch', scoreCap: 49, uiClass: 'bad', reason: 'Authoritative signal is TRIM WATCH; support labels are suppressed.' };
  if (signal.includes('INVESTIGATE')) return { state: 'investigate', label: 'Investigate', scoreCap: 54, uiClass: 'warn', reason: 'Authoritative signal is INVESTIGATE; support labels are suppressed until verified.' };
  return { state: 'normal', label: null, scoreCap: 100, uiClass: null, reason: null };
}
function scoreTrend(h) {
  ...
  return { score: clamp(50 + weighted * 2.4), finding: `Day ${fmtPct(day)}, 5D ${fmtPct(perf5d)}, 1M ${fmtPct(perf1m)}, 3M ${fmtPct(perf3m)}.` };
}
...
function scoreConcentration(h) {
  ...
  return { score: clamp(100 - Math.max(0, weight - cap) * 12 - (weight / cap) * 25), finding: `${weight.toFixed(2)}% versus ${cap}% ${speculative ? 'speculative/levered' : 'single non-index'} budget.` };
}
function tradingDaysUntil(value) {
  ...
  let days = 0; const cursor = new Date(today);
  while (cursor < target) { cursor.setDate(cursor.getDate() + 1); const d = cursor.getDay(); if (d !== 0 && d !== 6) days += 1; }
  return days;
}
function scoreEventRisk(h) {
  ...
  return { score: days == null ? 45 : days <= 5 ? 35 : days <= 15 ? 55 : 75, finding: days == null ? `Earnings date ${earnings}; timing unclear.` : `Next earnings in ${days} trading days.` };
}
...
function scoreMacro(h) {
  ...
  return { score: clamp(65 - negatives * 12 - intensity * 4 + Math.max(0, affected.length - negatives) * 4), finding: affected.map(force => `${force.name}: ${force.direction}`).join(' · ') };
}
...
function scorecard(h) {
  const authority = authoritativeSignal(h);
  const factors = [
    ['Price trend', scoreTrend(h)], ['Valuation', scoreValuation(h)], ['Portfolio concentration', scoreConcentration(h)], ['Earnings / event risk', scoreEventRisk(h)], ['Liquidity / flow', scoreLiquidity(h)], ['Macro sensitivity', scoreMacro(h)], ['Thesis confidence', scoreThesis(h)], ['Data completeness', scoreData(h)]
  ].map(([name, item]) => ({ name, score: Math.round(item.score), status: status(item.score), finding: item.finding }));
  let composite = Math.round(factors.reduce((sum, item) => sum + item.score, 0) / factors.length);
  composite = Math.min(composite, authority.scoreCap);
  const weakest = factors.slice().sort((a, b) => a.score - b.score)[0];
  const strongest = factors.slice().sort((a, b) => b.score - a.score)[0];
  return { ticker: h.ticker, signal: h.computedSignal || h.signal || 'Review', authority, composite, weakest, strongest, factors };
}
const scorecards = holdings.map(scorecard);
const warnings = scorecards.flatMap(card => card.factors.filter(factor => factor.score < 45).map(factor => ({ ticker: card.ticker, signal: card.signal, factor: factor.name, score: factor.score, finding: factor.finding }))).sort((a, b) => a.score - b.score);
const reviewQueue = scorecards.slice().sort((a, b) => a.composite - b.composite).slice(0, 5);
const strongest = scorecards.filter(card => card.authority.state === 'normal').slice().sort((a, b) => b.composite - a.composite).slice(0, 3);
...
function cardHtml(item, label) {
  ...
  return `<article class="portfolio-score-card"><span>${esc(labelFor(item, label))}</span><div><b>${esc(item.ticker)}</b><em class="${cardClass}">${item.composite}</em></div><p>${esc(item.authority?.reason || `${item.weakest?.name || item.factor}: ${item.weakest?.finding || item.finding || ''}`)}</p><a href="pages/${esc(String(item.ticker).toLowerCase())}.html">Open workspace →</a></article>`;
}
function warningHtml(item) { return `<article class="portfolio-warning"><div><b>${esc(item.ticker)}</b><span>${esc(item.factor)}</span><em class="bad">${item.score}</em></div><p>${esc(item.finding)}</p></article>`; }
...
const css = `<style>.portfolio-scoreboard{background:#ffffff}.portfolio-score-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:0;border-top:1px solid var(--rule);border-left:1px solid var(--rule)}.portfolio-score-card{border-right:1px solid var(--rule);border-bottom:1px solid var(--rule);padding:20px;background:#ffffff;min-height:210px}.portfolio-score-card span,.portfolio-warning span,.portfolio-score-foot span{display:block;color:var(--muted);font-size:13px;margin-bottom:10px}.portfolio-score-card div{display:flex;align-items:flex-start;justify-content:space-between;gap:14px}.portfolio-score-card b{font-size:34px;line-height:.95;letter-spacing:-.05em;font-weight:500}.portfolio-score-card em{font-style:normal;font-size:34px;line-height:.95;letter-spacing:-.05em}.portfolio-score-card p{margin:16px 0 0}.portfolio-score-card a{display:inline-block;margin-top:16px;text-decoration:none;font-size:14px;font-weight:500}.portfolio-score-card a:hover{text-decoration:underline;text-underline-offset:4px}.portfolio-warning-head{margin-top:28px}.portfolio-warning-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:0;border-top:1px solid var(--rule);border-left:1px solid var(--rule)}.portfolio-warning{border-right:1px solid var(--rule);border-bottom:1px solid var(--rule);padding:16px;background:#ffffff}.portfolio-warning div{display:grid;grid-template-columns:64px 1fr auto;gap:12px;align-items:start}.portfolio-warning b{font-size:20px;line-height:1;letter-spacing:-.03em}.portfolio-warning em{font-style:normal;font-weight:600}.portfolio-warning p{margin-top:12px;font-size:14px}.portfolio-score-foot{display:grid;grid-template-columns:1fr 1fr;gap:0;border-top:1px solid var(--rule);border-left:1px solid var(--rule);margin-top:22px}.portfolio-score-foot article{border-right:1px solid var(--rule);border-bottom:1px solid var(--rule);padding:18px;background:#ffffff}.portfolio-score-foot b{font-size:44px;line-height:.9;letter-spacing:-.05em;font-weight:500}@media(max-width:760px){.portfolio-warning div{grid-template-columns:1fr}.portfolio-score-foot{grid-template-columns:1fr}}</style>`;
...
if (!index.includes('portfolio-scoreboard')) { index = index.replace('</head>', `${css}</head>`); index = index.replace('<section id="holdings-section"', `${html}<section id="holdings-section"`); }
fs.writeFileSync(indexPath, index);
console.log(`Injected portfolio scoreboard: ${reviewQueue.length} review priorities, ${warnings.length} weak warnings; authoritative signals enforced`);
