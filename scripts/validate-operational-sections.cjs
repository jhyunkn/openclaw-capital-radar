...
const tickerGateAudit = fs.existsSync(path.join(root, 'outputs', 'ticker-gate-audit.json')) ? readJson('outputs/ticker-gate-audit.json') : null;
const tickerGateValidation = fs.existsSync(path.join(root, 'outputs', 'ticker-gate-audit-validation.json')) ? readJson('outputs/ticker-gate-audit-validation.json') : null;
...
const checks = [
  ...
  pass('Ticker Gate Audit', hasSection('ticker-gate-audit') && tickerGateAudit?.status === 'ACTIVE' && list(tickerGateAudit?.rows).length >= 20 && tickerGateValidation?.status === 'PASS', `${tickerGateAudit?.counts?.tickers || 0} tickers audited; ${tickerGateAudit?.counts?.pass || 0} pass; ${tickerGateAudit?.counts?.blocked || 0} blocked.`),
  pass('Public static sync', publicHtml.includes('live-reaction-state') && publicHtml.includes('opportunities-section') && publicHtml.includes('Intake / discovery') && publicHtml.includes('native-research-engine') && publicHtml.includes('opportunity-evidence-engine') && publicHtml.includes('ticker-gate-audit') && !publicHtml.includes('id="research-candidate-map"'), 'public/index.html includes live reaction, Native Research Engine, Ticker Gate Audit, Opportunity Evidence Engine, and unified Opportunity Scout sections.'),
