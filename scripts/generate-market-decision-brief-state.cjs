...
(async()=>{const op=read('operational-chart-state.json',{});const conf=read('confirmation-state.json',{});const bands=op.action_bands||{};const chart=op.chart||{};const [m2Rows,dgs10Rows,hyRows,dffRows]=await Promise.all([fred('M2SL'),fred('DGS10'),fred('BAMLH0A0HYM2'),fred('DFF')]);
