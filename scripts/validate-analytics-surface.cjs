...
function fail(message){ console.error(`ANALYTICS SURFACE VALIDATION FAILED: ${message}`); process.exit(1); }
function assert(condition,message){ if(!condition) fail(message); }
function readJson(file){ assert(fs.existsSync(file), `${path.relative(root,file)} missing`); return JSON.parse(fs.readFileSync(file,'utf8')); }
const state = readJson(statePath); const holdings = Array.isArray(state.holdings) ? state.holdings : [];
...
for (const h of holdings) {
  ...
  assert(index.includes(`>${String(h.ticker).toUpperCase()}<`) || index.includes(`>${h.ticker}<`), `compressed holdings matrix missing ${h.ticker}`);
...
console.log(`analytics surface validated: ${holdings.length} holding pages, compressed holdings homepage, JSON output`);
