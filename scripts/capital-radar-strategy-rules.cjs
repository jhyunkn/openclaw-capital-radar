const rules = {
  ...
  CEG: { posture:'AI power infrastructure hold-only / no-add', holdUntil:'Hold only while power-demand/data-center thesis remains intact and position size stays small.', addWhen:'Do not add by default; require a material expectation reset plus evidence that CEG beats stronger alternatives such as PWR, NXT, BWXT, CLS, or ET.', trimWhen:'Trim if AI-power narrative stays crowded, valuation outruns cash-flow, or better power/grid ideas offer cleaner asymmetry.', exitWhen:'Exit review if earnings/guidance contradicts AI-power thesis or relative strength breaks versus better infrastructure substitutes.', invalidation:'Power-demand upside fails to translate into durable cash-flow, or CEG becomes merely crowded narrative exposure.', review:'Earnings / power pricing / contracts / substitute ranking.' },
  VST: { posture:'AI power / electricity demand compounder watch', holdUntil:'Hold while AI/data-center electricity-demand thesis remains intact and valuation does not outrun power/cash-flow evidence.', addWhen:'Add only after pullback into defined buy zone or when power-price/contract evidence confirms upside without crowded-extension risk.', trimWhen:'Trim if power/AI narrative becomes euphoric, valuation outruns earnings/cash-flow, or combined VST+CEG power exposure exceeds risk budget.', exitWhen:'Exit review if data-center power thesis weakens, regulatory/merchant-power risk rises, or price breaks technical support on distribution volume.', invalidation:'Power-demand upside fails to translate into durable earnings/cash-flow or position becomes duplicate CEG exposure without better risk/reward.', review:'Earnings / power pricing / data-center contracts / regulatory headlines.' },
...
function profileFor(ticker){
  ...
  if (['CEG','NFLX','VST'].includes(ticker)) return 'watch';
  return 'core';
}
