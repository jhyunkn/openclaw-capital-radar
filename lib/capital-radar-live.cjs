...
async function fredLatest(id) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      if (apiKey) {
        const url = `https://api.stlouisfed.org/fred/series/observations?series_id=${encodeURIComponent(id)}&api_key=${apiKey}&file_type=json&sort_order=desc&limit=5`;
        ...
        return { id, name: fredSeries[id], source: 'FRED JSON API', sourceUrl: url, latestDate: obs?.date || null, value: Number.isFinite(value) ? round(value, 3) : null };
      } else {
