...
if (heroEnd >= 0) html = html.slice(0, heroEnd + 10) + section + html.slice(heroEnd + 10);
else if (/<main[^>]*>/.test(html)) html = html.replace(/(<main[^>]*>)/, `$1${section}`);
else html = html.replace(/(<body[^>]*>)/, `$1${section}`);
fs.writeFileSync(indexPath, html);
console.log('injected evidence annotation layer after hero');
