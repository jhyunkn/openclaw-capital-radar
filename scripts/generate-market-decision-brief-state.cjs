...
(async()=>{const op=read('operational-chart-state.json',{});const conf=read('confirmation-state.json',{});const tape=read('market-tape-state.json',{});const liveState=read('report-state.live.json',{});const liveRate=id=>{const rows=Array.isArray(liveState.liveRatesCredit)?liveState.liveRatesCredit:[];const r=rows.find(x=>x&&x.id===id);return r&&num(r.value)!==null?{value:num(r.value),date:r.date}:null};const bands=op.action_bands||{};const chart=op.chart||{};const [m2Rows,dgs10Rows,hyRows,dffRows]=await Promise.all([fred('M2SL'),fred('DGS10'),fred('BAMLH0A0HYM2'),fred('DFF')]);
const m2=latest(m2Rows);const dgs10=latest(dgs10Rows)||liveRate('DGS10'),hy=latest(hyRows)||liveRate('BAMLH0A0HYM2'),dff=latest(dffRows)||liveRate('DFF');const m2YoY=yoy(m2Rows);const tapeDgs10=rateFromTape(tapeValue(tape,'Rates'));const tapeHy=spreadFromTape(tapeValue(tape,'Credit'))??spreadFromTape(tapeValue(tape,'Liquidity'));const spx=num(bands.current);const ma50=num(chart.overlays?.ma50);const ma200=num(chart.overlays?.ma200);const vix=num(chart.indicators?.vix);const rsi=num(chart.indicators?.rsi14);
...
const macro_values=cleanMacroValues([
 ...
 {key:'dgs10',label:'10Y Treasury',value:dgs10Value,unit:'%',type:dgs10?.value!==undefined?'REAL':'DERIVED',source:dgs10?.value!==undefined?'FRED DGS10 / live adapter':'market-tape fallback',date:dgs10?.date},
