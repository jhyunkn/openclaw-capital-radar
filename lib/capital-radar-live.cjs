...
function buildOpportunityScout(state) {
  const ideas = [
    { theme: 'AI infrastructure picks-and-shovels', candidateType: 'watchlist basket', whyNow: 'Force remains active, but crowding means wait for valuation reset or earnings-confirmed durability.', score: 72, signal: 'ADD WATCH' },
    { theme: 'Quality compounders after rate-driven pullback', candidateType: 'screen', whyNow: 'If rates pressure strong businesses without thesis damage, opportunity may improve.', score: 76, signal: 'ADD WATCH' },
    { theme: 'Credit-stress beneficiaries / defensive quality', candidateType: 'screen', whyNow: 'Useful if risk appetite weakens but portfolio needs durable cash-flow exposure.', score: 68, signal: 'ADD WATCH' },
    { theme: 'Crypto beta replacement with less decay', candidateType: 'risk substitution', whyNow: 'CONL-style levered decay should be compared against cleaner exposure vehicles.', score: 61, signal: 'INVESTIGATE' },
    { theme: 'Consumer resilience names', candidateType: 'screen', whyNow: 'MA, AMZN, NFLX exposures need consumer pressure/cash-flow comparison.', score: 64, signal: 'INVESTIGATE' }
  ];
...
function buildStrategyState(state, live) {
  ...
  return {
    ...
    marketForces,
    opportunityScout: buildOpportunityScout(state),
