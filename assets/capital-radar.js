...
function renderForces(state){
  $('forces').innerHTML = (state.strategy.marketForces || []).map(f => `<article class="force">
    <div class="force-top"><b>${f.name}</b><span>${f.direction}</span></div>
    <div class="intensity"><i style="width:${Math.min(100, (f.intensity || 1) * 20)}%"></i></div>
    <p>${f.interpretation}</p>
    <p class="muted">Affected: ${(f.affected || []).join(', ')}</p>
  </article>`).join('');
}

function renderOpportunities(state){
  $('opportunities').innerHTML = (state.strategy.opportunityScout || []).map(o => `<article class="card opportunity">
    <div class="ticker"><div><b>${o.score}</b><br><small>Blueprint score</small></div><span class="signal ${o.signal.includes('ADD')?'good':'warn'}">${o.signal}</span></div>
    <h3>${o.theme}</h3>
    <p>${o.whyNow}</p>
    <p class="muted">Type: ${o.candidateType}</p>
  </article>`).join('');
}
...
function render(state){
  ...
  renderExposure(state); renderForces(state); renderOpportunities(state); renderHoldings(state); renderMarket(state); renderRates(state); renderSources(state);
