...
const concentrationTop = rows.slice(0,4).map(h=>`${h.ticker} ${h.weight.toFixed(1)}%`).join(' + ');
const concentrationTotal = rows.slice(0,4).reduce((s,h)=>s+h.weight,0).toFixed(1);
...
function strategyHtml(ticker){
  const r = strategyFor(ticker);
  return `<div class="strategy"><div><b>Hold until</b><p>${e(r.holdUntil)}</p></div><div><b>Add when</b><p>${e(r.addWhen)}</p></div><div><b>Trim when</b><p>${e(r.trimWhen)}</p></div><div><b>Exit when</b><p>${e(r.exitWhen)}</p></div></div>`;
}
function money(n){ return typeof n === 'number' ? `${n < 0 ? '-' : '+'}$${Math.abs(n).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}` : 'n/a'; }
function pct(n){ return typeof n === 'number' ? `${n < 0 ? '-' : '+'}${Math.abs(n).toFixed(2)}%` : 'n/a'; }
function returnTone(n){ return typeof n === 'number' && n < 0 ? 'danger' : 'good'; }
function cardList(items, limit=6){
  return items.slice(0,limit).map(x=>`<li>${e(x)}</li>`).join('');
}
const topActions = [
  {k:'Read first', v:`Top 4 concentration: ${concentrationTop} = ${concentrationTotal}%`, c:'warn'},
