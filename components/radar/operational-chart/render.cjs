...
function renderOperationalChartStyle() {
  return `<style id="operational-chart-style">.op-lwc-chart{height:520px;border:1px solid var(--rule);border-radius:0;background:#ffffff;overflow:hidden;margin:0 0 20px}.op-chart-section .section-head{margin-bottom:18px}.macro-price-strip{display:grid;grid-template-columns:repeat(auto-fit,minmax(118px,1fr));gap:8px}.mp-tile{border:1px solid var(--rule);border-radius:0;background:#ffffff;padding:13px}.mp-label{display:block;font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);font-weight:500}.mp-price{display:block;font-size:17px;line-height:1.05;letter-spacing:-.025em;font-weight:600;margin-top:10px}.mp-chg{display:block;font-size:11px;margin-top:6px;color:var(--muted)}.mp-chg.up{color:var(--green,#2f6f4e)}.mp-chg.dn{color:var(--red,#9f3f35)}@media(max-width:900px){.macro-price-strip{grid-template-columns:repeat(3,1fr)}.op-lwc-chart{height:380px}}@media(max-width:560px){.macro-price-strip{grid-template-columns:repeat(2,1fr)}.op-lwc-chart{height:300px}}</style>`;
}
