...
const section = `<section class="current-market-state" id="current-market-state"><div class="cms-wrap"><div class="cms-head"><div><p class="cms-kicker">Current market state · documented source charts</p><h2 class="cms-title">The market speaks in time series before Capital Radar interprets.</h2><p class="cms-copy">This surface preserves the chart logic used by financial institutions: time on the x-axis, observed value on the y-axis, current point annotated. Percentile, direction, freshness, and basis remain metadata, not replacement visuals.</p></div><aside class="cms-meta"><span>As of</span><b>${esc(state.asOf || state.generatedAt || 'unknown')}</b><span style="margin-top:14px">Visual rule</span><b>Axis-aligned</b></aside></div><div class="cms-grid">${groups.map(groupMarkup).join('')}</div><div class="cms-anomaly-row">${anomalies.map(anomalyMarkup).join('')}</div></div></section>`;
