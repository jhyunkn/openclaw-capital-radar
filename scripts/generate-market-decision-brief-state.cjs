...
(async()=>{const op=read('operational-chart-state.json',{});const bands=op.action_bands||{};const chart=op.chart||{};const [m2Rows,dgs10Rows,hyRows,dffRows]=await Promise.all([fred('M2SL'),fred('DGS10'),fred('BAMLH0A0HYM2'),fred('DFF')]);
const m2=latest(m2Rows),dgs10=latest(dgs10Rows),hy=latest(hyRows),dff=latest(dffRows);const m2YoY=yoy(m2Rows);const spx=num(bands.current);const ma50=num(chart.overlays?.ma50);const ma200=num(chart.overlays?.ma200);const vix=num(chart.indicators?.vix);const rsi=num(chart.indicators?.rsi14);
let market='SPX trend is intact but not a blind add setup.';if(spx&&ma50&&spx<ma50)market='SPX is below/near 50D: trend is weakening and confirmation matters.';if(spx&&ma200&&spx<ma200)market='SPX is below 200D: regime defense takes priority.';if(spx&&ma50&&spx>ma50&&vix!==null&&vix<20)market='SPX is above 50D with volatility contained: hold core, add only on ruled pullbacks.';
let macro='Macro backdrop is mixed.';if(dgs10?.value>=4.5)macro='Rates remain a valuation headwind for long-duration growth.';else if(dgs10?.value<4)macro='Rates are less restrictive and support duration-sensitive assets.';if(m2YoY!==null&&m2YoY>4)macro+=` M2 is expanding YoY, supporting liquidity.`;else if(m2YoY!==null&&m2YoY<0)macro+=` M2 remains contracting YoY, limiting liquidity beta.`;
const action=op.brief?.portfolio_posture||'Hold core / no broad chase';const trigger=op.brief?.change_trigger||'Use chart bands for add/trim/defense triggers.';
const brief=`${market} ${macro} Current posture: ${action}. Change rule: ${trigger}`;
const macro_values=[
 ...
 {key:'dgs10',label:'10Y Treasury',value:round(dgs10?.value,2),unit:'%',type:'REAL',source:'FRED DGS10',date:dgs10?.date},
 ...
 {key:'hy_oas',label:'HY OAS',value:round(hy?.value,2),unit:'spread',type:'REAL',source:'FRED BAMLH0A0HYM2',date:hy?.date},
 {key:'rsi14',label:'RSI 14',value:round(rsi,1),unit:'',type:'EST',source:'derived from SPX closes'}
];
const state={as_of:new Date().toISOString(),artifact:'market-decision-brief-state',brief,market_read:market,macro_read:macro,portfolio_action:action,change_rule:trigger,risk_rule:op.brief?.risk_trigger||null,macro_values,chart_reference:{symbol:op.symbol||'^GSPC',current:spx,ma50:round(ma50),ma200:round(ma200),add_zone:bands.add_zone,trim_zone:bands.trim_zone,defense_below:bands.defense_below,hard_risk:bands.hard_risk},render_permission:true};
write('market-decision-brief-state.json',state);console.log('market decision brief state generated');
