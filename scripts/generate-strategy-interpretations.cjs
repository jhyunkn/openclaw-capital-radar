...
function riskCap(h) { const type = exposureType(h); if (type === 'index') return 55; return type === 'levered' || type === 'crypto' ? 5 : 15; }
function capFramework(h) { const type = exposureType(h); if (type === 'index') return 'Index ballast framework'; if (type === 'levered') return 'Levered / decay risk framework'; if (type === 'crypto') return 'Crypto / liquidity-beta framework'; return 'Single-name concentration framework'; }
...
function portfolioConflict(h) {
  ...
  if (ticker === 'SPY' && n(h.portfolioWeightPct) >= 50) return { status: 'Index ballast concentration', tone: 'caution', reason: `SPY is ${n(h.portfolioWeightPct).toFixed(2)}% of the account; acceptable only under index ballast framework, not single-name logic.` };
...
function actionPermission(h, pressure, thesis, data) {
  ...
  if (pressure.tone === 'danger') return { status: 'Hold only', tone: 'caution', reason: `Position is ${pressure.status.toLowerCase()} under ${pressure.framework}; adding is blocked by risk budget.` };
...
function signalChangeConditions(h, boundary, pressure, data) {
  ...
  if (pressure.tone === 'danger') conditions.push('Adding remains blocked until position pressure is reduced.');
...
function interpret(h) {
  ...
  return { ticker: h.ticker, signal: h.computedSignal || h.signal || 'Review', role: role(h), exposureType: exposureType(h), riskCap: riskCap(h), capFramework: capFramework(h), thresholdPolicy: h.signalThresholds?.formula || 'missing_signal_thresholds', thesisStatus: thesis, actionPermission: action, urgency: urgent, newInformation: newInformation(h, boundary, pressure, data, trend, conflict), nearestDecisionBoundary: boundary, positionPressure: pressure, trendRead: trend, dataConfidence: data, portfolioConflict: conflict, decisionConfidence: confidence, signalChangeConditions: signalChangeConditions(h, boundary, pressure, data) };
}
const interpretations = holdings.map(interpret);
const homepage = { actNow: interpretations.filter(x => x.urgency.level === 'Now'), reviewSoon: interpretations.filter(x => ['This week','Soon'].includes(x.urgency.level)), blockedAdds: interpretations.filter(x => /No action|No add|Hold only|verify/i.test(x.actionPermission.status)), weakData: interpretations.filter(x => x.dataConfidence.tone !== 'positive'), portfolioConflicts: interpretations.filter(x => x.portfolioConflict.tone !== 'positive'), highConfidence: interpretations.filter(x => x.decisionConfidence.level === 'High') };
fs.mkdirSync(path.dirname(outPath), { recursive: true });
fs.writeFileSync(outPath, JSON.stringify({ generatedAt: new Date().toISOString(), policy: 'Strategy Interpreter uses h.signalThresholds as the authoritative signal-change source. Index instruments use the index ballast cap framework rather than the single-name cap.', homepage, interpretations }, null, 2) + '\n');
