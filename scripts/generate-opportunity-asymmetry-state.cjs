...
function opportunityRead(packet, gate) { const stage = String(packet.opportunityStage || 'research').toLowerCase(); const price = text(packet.priceFrame?.priceRead, 'price context pending'); const thesis = text(packet.whyInteresting, 'thesis pending'); if (gate.evidence_completeness_pct >= 75) return 'promotion_candidate'; if (/pullback|dislocation|meaningful/i.test(price)) return 'dislocation_watch'; if (/infrastructure|power|grid|cooling|data-center/i.test(thesis)) return 'second_order_infrastructure_watch'; if (/quality|compounder|valuation/i.test(thesis)) return 'quality_reset_watch'; return stage.replace(/[^a-z0-9]+/g, '_') || 'research_watch'; }
...
function promotionStatus(packet, gate) { if (gate.evidence_completeness_pct >= 75) return 'promotion_review'; if (gate.evidence_completeness_pct >= 45) return 'build_evidence_packet'; return baseStatus(packet); }
...
for (const packet of packets) {
  ...
  const candidate = {
    ...
    opportunity_read: opportunityRead(packet, gate),
    ...
    promotion_status: promotionStatus(packet, gate),
...
const output = { as_of: now, cycle_id: cycleId, landscape_cycle_id: landscape.cycle_id || null, purpose: 'Map macro direction into underpriced second-order ticker expressions with canonical evidence-gate completeness and promotion blockers. Buy permission is a guardrail, not the opportunity status.', opportunity_clusters: clusters, summary: { clusters: clusters.length, candidates: all.length, priority_research: all.filter(x => x.promotion_status === 'priority_research').length, build_evidence_packet: all.filter(x => x.promotion_status === 'build_evidence_packet').length, promotion_review: all.filter(x => x.promotion_status === 'promotion_review').length, watch_and_compare: all.filter(x => x.promotion_status === 'watch_and_compare').length, average_evidence_completeness_pct: all.length ? Number((all.reduce((s, x) => s + x.evidence_completeness_pct, 0) / all.length).toFixed(1)) : 0, changed_since_last_cycle: clusters.filter(c => c.changed_since_last_cycle).length }, render_permission: clusters.length > 0 && clusters.every(c => c.macro_theme && c.second_order_direction && c.evidence_ids.length && c.candidate_tickers.every(t => t.ticker && t.missing_evidence.length && t.promotion_status && Number.isFinite(t.evidence_completeness_pct))) };
