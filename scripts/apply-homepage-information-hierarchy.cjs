...
const hierarchyCss = `<style>.information-hierarchy{background:#ffffff}.hierarchy-steps{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:0;border-top:1px solid var(--rule);border-left:1px solid var(--rule)}.hierarchy-steps article{padding:16px;border-right:1px solid var(--rule);border-bottom:1px solid var(--rule);background:#ffffff;min-height:168px}.hierarchy-steps span{display:block;color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.06em;margin-bottom:14px}.hierarchy-steps b{display:block;font-size:20px;line-height:1.05;letter-spacing:-.035em}.hierarchy-steps p{font-size:13px;line-height:1.42;margin-top:10px;color:rgba(36,35,31,.78)}@media(max-width:1000px){.hierarchy-steps{grid-template-columns:1fr 1fr}}@media(max-width:640px){.hierarchy-steps{grid-template-columns:1fr}}</style>`;
