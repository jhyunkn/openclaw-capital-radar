...
<section id="market-orientation" class="market-orientation-surface" data-orientation-map="true">
...
const briefEnd = html.match(/<section[^>]*id="brief"[\s\S]*?<\/section>/);
if (briefEnd) {
  html = html.replace(briefEnd[0], `${briefEnd[0]}${orientationHtml}`);
} else {
  html = html.replace(/<main[^>]*>/, match => `${match}${orientationHtml}`);
}

fs.writeFileSync(indexPath, html);
console.log(`injected market orientation homepage surface: ${list(map.themes).length} themes`);
