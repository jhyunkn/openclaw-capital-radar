...
for (const file of fs.readdirSync(pagesDir).filter(x => x.endsWith('.html'))) {
  ...
  html = html.replace(/<style>\.thesis-dossier[\s\S]*?<\/style>/, '');
  html = html.replace(/<section class="section thesis-dossier">[\s\S]*?<\/section>(?=\s*<section|\s*<\/main>)/, '');
  html = html.replace('</head>', `${css}</head>`);
  const chartMatch = html.match(/<section class="section chart-section">[\s\S]*?<\/section>/);
  if (chartMatch) html = html.replace(chartMatch[0], `${chartMatch[0]}${section(d)}`);
  else html = html.replace('</main>', `${section(d)}</main>`);
