...
index = index.replace(/<div class="live-reaction-panel[\s\S]*?<\/div>\s*(?=<section|<\/section|<\/main>)/, '');
index = index.replace('</head>', `${css}</head>`);
// Embed inside holdings section (compressed layout) or before holdings-section (operational layout)
const anchors = [
...
for (const anchor of anchors) {
  if (index.includes(anchor)) { index = index.replace(anchor, `${html}${anchor}`); placed = true; break; }
}
if (!placed) index = index.replace('</main>', `${html}</main>`);
