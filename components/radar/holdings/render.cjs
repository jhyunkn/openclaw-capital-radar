...
function fundamentalsRow(holding) {
  ...
  const news = holding.high_materiality_news;
  const newsHtml = news?.length
    ? `<div class="zone-news-alert"><b>NEWS</b> ${esc(news[0].headline?.slice(0, 65))}… <span>${esc(news[0].materiality_signals?.join('/'))}</span></div>`
    : '';
  if (!parts.length && !newsHtml) return '';
  return `<div class="zone-fundamentals">${parts.join('')}</div>${newsHtml}`;
}
