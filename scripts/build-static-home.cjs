...
function holdingsHtml(){
  return list(state.holdings).map(h => `<article class="card">
    ...
    <a class="detail-link" href="pages/${esc(String(h.ticker || '').toLowerCase())}.html">Open ${esc(h.ticker)} rating + chart →</a>
...
function forcesHtml(){
  return list(state.strategy?.marketForces).map(f => `<article class="force"><div class="force-top"><b>${esc(f.name)}</b><span>${esc(f.direction)}</span></div><div class="intensity"><i style="width:${Math.min(100, (f.intensity || 1) * 20)}%"></i></div><p>${esc(f.interpretation)}</p><p class="muted">Affected: ${esc(list(f.affected).join(', ') || 'n/a')}</p></article>`).join('') || '<p class="muted">No market force map loaded.</p>';
}
