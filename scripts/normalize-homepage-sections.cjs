...
// Production visual pass: order the homepage as a macro-to-action cockpit.
// Current landscape first, framework diagram later.
...
const navHtml=ids.filter(id=>labels[id]&&source.includes(`id="${id}"`)).map(id=>`<a href="#${id}">${labels[id]}</a>`).join('');
...
fs.writeFileSync(indexPath,html);console.log('homepage section normalization complete: macro-to-action order');