...
const fredCachePath = path.join(__dirname, '..', 'outputs', 'fred-rates-cache.json');
const fredSeries = {
...
const secCandidateSymbols = ['MSFT','AMZN','CEG','META','MA','NFLX','TSLA','COIN','AVGO','VRT','GEV','ETN','PWR','NXT','CCJ','OKLO','SMR','RKLB','PLTR','RDDT','HIMS','TMDX','DT','TTD'];
...
function readFredCache() {
  const cache = {};
  try {
    const cached = JSON.parse(fs.readFileSync(fredCachePath, 'utf8'));
    for (const row of cached.series || []) {
      if (row?.id && row.value != null) cache[row.id] = row;
    }
  } catch {}
  try {
    const report = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'data', 'report-state.live.json'), 'utf8'));
    for (const row of report.liveRatesCredit || []) {
      if (row?.id && row.value != null && !cache[row.id]) cache[row.id] = row;
    }
  } catch {}
  return cache;
}
function writeFredCache(rows) {
  const freshRows = (rows || []).filter(row => row?.id && row.value != null && !row.fetchedFromCache);
  if (!freshRows.length) return;
  // Merge: keep last-known-good values for series that did not refresh this
  // run. Replacing the whole file would permanently drop cached series
  // whenever only a subset fetches fresh (seen 2026-09-25: 7 -> 2).
  const merged = readFredCache();
  for (const row of freshRows) merged[row.id] = row;
  const payload = JSON.stringify({
    generatedAt: new Date().toISOString(),
    source: 'Capital Radar successful FRED fetch cache',
    series: Object.values(merged)
  }, null, 2) + '\n';
  try {
    fs.mkdirSync(path.dirname(fredCachePath), { recursive: true });
    fs.writeFileSync(fredCachePath, payload);
  } catch (error) {
    if (error && (error.code === 'EROFS' || error.code === 'EACCES')) return;
    throw error;
  }
}
async function fetchJson(url) {
  const res = await fetch(url, { headers: { 'user-agent': 'OpenClaw Capital Radar/1.0 jun.hn.nam@gmail.com' } });
...
async function fredLatest(id) {
  ...
  const maxAttempts = Number(process.env.FRED_FETCH_RETRIES || 3);
  const timeoutMs = Number(process.env.FRED_FETCH_TIMEOUT_MS || 15000);
  const publicApiUrl = `https://api.stlouisfed.org/fred/series/observations?series_id=${encoded}&file_type=json&sort_order=desc&limit=5`;
  const csvUrl = `https://fred.stlouisfed.org/graph/fredgraph.csv?id=${encoded}`;
  let lastError;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (apiKey) {
        const requestUrl = `${publicApiUrl}&api_key=${encodeURIComponent(apiKey)}`;
        const res = await fetch(requestUrl, { headers: { 'user-agent': 'OpenClaw Capital Radar research prototype' }, signal });
        if (!res.ok) throw new Error(`${res.status} ${publicApiUrl}`);
        const json = await res.json();
        const obs = (json.observations || []).find(o => o.value !== '.');
        const value = obs ? Number(obs.value) : null;
        return { id, name: fredSeries[id], source: 'FRED JSON API', sourceUrl: publicApiUrl, latestDate: obs?.date || null, value: Number.isFinite(value) ? round(value, 3) : null };
      } else {
        const res = await fetch(csvUrl, { headers: { 'user-agent': 'OpenClaw Capital Radar research prototype' }, signal });
        if (!res.ok) throw new Error(`${res.status} ${csvUrl}`);
        const latest = parseFredCsv(await res.text());
        return { id, name: fredSeries[id], source: 'FRED CSV public endpoint', sourceUrl: csvUrl, latestDate: latest?.date || null, value: latest ? round(latest.value, 3) : null };
      }
    } catch (e) {
      lastError = e;
...
// FRED's public CSV endpoint has been unreachable from datacenter IPs since
// 2026-09-18 (hangs until timeout; main site is fine). Yahoo's chart API is
// live, so use its Treasury-yield indices as the fallback for the two yields
// it carries. DGS2 / T10YIE / credit spreads / DFF have no Yahoo equivalent
// and still need a FRED API key (FRED_API_KEY) or the cache.
const fredYahooFallback = { DGS10: '^TNX', DGS30: '^TYX' };

async function fredLatestLive(id) {
  try {
    return await fredLatest(id);
  } catch (fredError) {
    const yahooSym = fredYahooFallback[id];
    if (!yahooSym) throw fredError;
    const chart = await yahooChart(yahooSym);
    if (!Number.isFinite(chart.price)) throw fredError;
    return {
      id,
      name: fredSeries[id],
      source: `Yahoo Finance ${yahooSym} yield index (FRED CSV unreachable since 2026-09-18)`,
      sourceUrl: chart.sourceUrl,
      latestDate: String(chart.asOf || '').slice(0, 10) || null,
      value: round(chart.price, 3),
    };
  }
}
...
function attachPrices(state, live) {
  ...
  const priced = state.holdings.map(h => {
    ...
    // Record what the pipeline actually knows: a live quote this run earns medium
    // price confidence (fundamentals are not refreshed here); no quote stays
    // 'missing' so the deploy gate keeps failing closed on absent data.
    const enriched = { ...h, livePrice: quote?.price ?? null, priceAsOf: quote?.asOf ?? null, dayChangePct: quote?.changePct ?? null, perf5dPct: quote?.perf5dPct ?? null, perf1mPct: quote?.perf1mPct ?? null, perf3mPct: quote?.perf3mPct ?? null, sparkline: quote?.sparkline ?? [], marketValue: marketValue != null ? round(marketValue, 2) : null, liveDataSource: quote?.source ?? 'unavailable', exposureBucket: exposureBucket(h), fiftyTwoWeekHigh: quote?.fiftyTwoWeekHigh ?? null, fiftyTwoWeekLow: quote?.fiftyTwoWeekLow ?? null, regularMarketVolume: quote?.regularMarketVolume ?? null, sourceConfidence: h.sourceConfidence || h.confidence || (quote?.price != null ? 'medium: live Yahoo Finance quote; fundamentals not refreshed this run' : 'missing') };
    return enriched;
  });
...
function buildOpportunityScout(state, live) {
  ...
  const ideas = [
    idea({
      ticker: 'CLS',
      name: 'Celestica',
      theme: 'AI hardware manufacturing',
      thesis: 'Less obvious AI infrastructure exposure through contract manufacturing for hyperscaler servers, networking, and storage.',
      whyNow: 'Better asymmetry than adding the obvious GPU winner if revenue acceleration and guide raises keep confirming AI hardware demand.',
      confirmBeforeAdd: ['AI hardware revenue growth remains above 20%', 'Customer concentration risk is understood', 'FCF and guide quality confirm the demand is not one quarter of pull-forward'],
      keyRisks: ['Customer concentration', 'Thin contract-manufacturing margins', 'Tariff and supply-chain exposure'],
      score: 84,
      signal: 'ADD WATCH'
    }),
    idea({ ticker: 'BWXT', name: 'BWX Technologies', theme: 'Defense nuclear infrastructure', thesis: 'Mandated naval nuclear component exposure with contracted backlog; power/nuclear adjacency without relying on the crowded CEG/NVDA narrative.', whyNow: 'A better research lane than CEG if backlog conversion and defense awards keep showing durable, non-speculative demand.', confirmBeforeAdd: ['Backlog converts to revenue', 'Defense budget risk remains contained', 'SMR optionality is not treated as the base case'], keyRisks: ['Defense budget cuts', 'Execution on backlog', 'Valuation if growth stalls'], score: 83, signal: 'ADD WATCH' }),
    idea({ ticker: 'ET', name: 'Energy Transfer LP', theme: 'AI infrastructure income', thesis: 'Midstream gas and pipeline exposure tied to data-center power demand, with income while waiting for the AI-energy narrative to broaden.', whyNow: 'Differentiated from the crowded AI/power equity basket and potentially more cash-flow anchored.', confirmBeforeAdd: ['DCF trend confirms contract ramp', 'MLP/K-1 account fit is acceptable', 'Distribution safety remains intact'], keyRisks: ['Leverage', 'MLP tax complexity', 'Gas demand/capex slowdown'], score: 81, signal: 'ADD WATCH' }),
    idea({ ticker: 'KNSL', name: 'Kinsale Capital', theme: 'Non-correlated quality compounder', thesis: 'Best-in-class specialty insurer that adds a boring, high-quality compounding lane outside AI crowding.', whyNow: 'Useful when the watchlist is too concentrated in AI infrastructure and power narratives.', confirmBeforeAdd: ['Combined ratio remains best-in-class', 'Premium growth does not break below zero', 'Catastrophe/loss risk is sized correctly'], keyRisks: ['E&S pricing softening', 'Catastrophe year', 'Insurance names staying ignored in an AI tape'], score: 80, signal: 'ADD WATCH' }),
    idea({
      ticker: 'AVGO',
      name: 'Broadcom',
    ...
    idea({ ticker: 'NVDA', name: 'NVIDIA', theme: 'AI compute benchmark', thesis: 'Benchmark only: the business quality is obvious, but watchlist asymmetry is weaker because everyone already knows the AI accelerator thesis.', whyNow: 'Use it to validate AI capex health, not as a scarce top idea unless a real dislocation appears.', confirmBeforeAdd: ['Major valuation reset', 'Demand/gross-margin durability remains intact', 'Export-control and customer concentration risks are understood'], keyRisks: ['Consensus crowding', 'Multiple compression', 'AI capex digestion cycle'], score: 58, signal: 'INVESTIGATE' }),
    idea({ ticker: 'ETN', name: 'Eaton', theme: 'Electrical systems / data-center power spine', thesis: 'Industry comp, not a priority idea: the setup is weaker than more direct or less crowded infrastructure names.', whyNow: 'Keep as a relative-strength/valuation comp against PWR, NXT, BWXT, CLS, and ET rather than a promoted candidate.', confirmBeforeAdd: ['Material valuation reset', 'Data-center/electrical growth is clearly underpriced', 'Relative setup beats higher-asymmetry alternatives'], keyRisks: ['Valuation premium', 'Industrial slowdown', 'Theme overcrowding'], score: 54, signal: 'INVESTIGATE' }),
...
async function buildLiveState(options = {}) {
  ...
  // Load XBRL fundamentals once — attach to holdings below
  let xbrlByTicker = {};
  try {
    const xbrlPath = path.join(__dirname, '..', 'outputs', 'sec-xbrl-fundamentals.json');
    if (fs.existsSync(xbrlPath)) {
      const xbrl = JSON.parse(fs.readFileSync(xbrlPath, 'utf8'));
      for (const r of (xbrl.results || [])) {
        if (r.ticker && r.fundamentals) xbrlByTicker[r.ticker.toUpperCase()] = r.fundamentals;
      }
    }
  } catch {}
  ...
  await Promise.all(Object.keys(fredSeries).map(async id => {
    try {
      ratesCredit.push(await fredLatestLive(id));
    } catch (e) {
      const cached = fredCache[id];
      if (cached?.value != null) {
        ratesCredit.push({
          ...cached,
          name: cached.name || fredSeries[id],
          source: `${cached.source || 'FRED'} (cached fallback)`,
          fetchedFromCache: true,
          cacheFallbackReason: e.message
        });
      } else {
        errors.push({ series: id, error: e.message });
      }
    }
  }));
  writeFredCache(ratesCredit);
  ...
  const usedFredCacheFallback = ratesCredit.some(row => row.fetchedFromCache);
  state.meta.reportDate = todayISO();
  state.meta.dataStatus = errors.length || usedFredCacheFallback ? 'PARTIAL_LIVE_PUBLIC_DATA' : 'LIVE_PUBLIC_DATA';
  state.meta.generatedAt = new Date().toISOString();
  state.meta.liveDataSources = ['Yahoo Finance chart API public/unofficial endpoint', process.env.FRED_API_KEY ? 'FRED JSON API' : 'FRED public CSV endpoint', 'SEC submissions JSON public endpoint'];
  state.marketRegime = classifyRegime({ marketTape, ratesCredit });
  state.holdings = attachPrices(state, { marketTape, ratesCredit });
  // Holdings freshness stamp: share counts are manual (no Robinhood auto-sync per
  // mandate); holding prices are live-proxy refreshed here each operating loop via
  // Yahoo Finance. Stamp the price refresh as the holdings sync time so the
  // freshness validator measures what the pipeline actually renews. The last true
  // position-count sync stays recorded in holdingsSyncNote for honesty.
  {
    const pricedCount = state.holdings.filter(h => h.livePrice != null && Number.isFinite(Number(h.livePrice)) && Number(h.livePrice) > 0).length;
    if (pricedCount > 0) {
      state.meta.holdingsSyncedAt = new Date().toISOString();
      state.meta.holdingsSyncNote = 'Share counts are manual (last Robinhood position sync 2026-09-04); holding prices live-proxy refreshed via Yahoo Finance each operating loop.';
    }
  }
  // Attach portfolio_role and XBRL fundamentals to each holding
  state.holdings = state.holdings.map(h => {
    const xbrl = xbrlByTicker[h.ticker] || null;
    return {
      ...h,
      portfolio_role: h.role || 'MISSING',
      xbrl_fundamentals: xbrl ? {
        revenue_ttm_usd_millions:  xbrl.revenue_ttm_usd_millions,
        revenue_growth_pct:        xbrl.revenue_growth_pct,
        gross_margin_pct:          xbrl.gross_margin_pct,
        net_income_usd_millions:   xbrl.net_income_usd_millions,
        fcf_usd_millions:          xbrl.fcf_usd_millions,
        eps_diluted:               xbrl.eps_diluted,
        long_term_debt_usd_millions: xbrl.long_term_debt_usd_millions,
        dilution_pct_yoy:          xbrl.dilution_pct_yoy,
