...
const checks = [
  ...
  pass('Opportunity', html.includes('id="opportunity"') && list(opportunityPackets.priorityQueue).length >= 3 && !html.includes('Opportunity Evidence Engine'), `${list(opportunityPackets.priorityQueue).length} priority opportunity packets compressed into one operator-facing section.`),
  ...
  pass('Homepage constitution output', constitution.legacyHomepageSectionsRemoved === true && JSON.stringify(constitution.canonicalSections) === JSON.stringify(expected), 'Canonical homepage constitution JSON written for ChatGPT/OpenClaw pickup.'),
